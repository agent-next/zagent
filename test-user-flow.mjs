...
import { readFileSync, mkdirSync, rmSync, symlinkSync, copyFileSync } from 'node:fs';
...
//    sandboxed HOME (copy of the real config + .local symlink) so TUI side-effects
//    can never mutate the developer's ~/.zcode (2026-09-05 contamination incident).
const ws = '/tmp/zuf-ws';
rmSync(ws, { recursive: true, force: true }); mkdirSync(ws, { recursive: true });
const zhome = '/tmp/zuf-zhome';
rmSync(zhome, { recursive: true, force: true }); mkdirSync(`${zhome}/.zcode/cli`, { recursive: true });
try {
  symlinkSync(`${os.homedir()}/.local`, `${zhome}/.local`);
  copyFileSync(`${os.homedir()}/.zcode/cli/config.json`, `${zhome}/.zcode/cli/config.json`);
} catch {}
const t = spawnSync('bash', ['-c',
  ...
  { encoding: 'utf8', env: { ...process.env, HOME: zhome }, timeout: 90000, maxBuffer: 32e6 });
const out = t.stdout ?? '';
...
const home2 = '/tmp/zuf-home';
rmSync(home2, { recursive: true, force: true }); mkdirSync(`${home2}/.zcode/cli`, { recursive: true });
