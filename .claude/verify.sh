...
# canonical serial gate CI runs), discovered via the same discoverTests module
# — but executed in parallel so the whole suite fits the hook's ~100s budget
# (serial: ~152s). Any drift here is caught by CI's serial run of test-all.
set -uo pipefail
...
// they cannot fit the hook's ~100s budget. The commit gate runs everything else;
// CI's serial `node scripts/test-all.mjs` remains the full-fidelity gate for these.
...
console.log(`\n${run.length - failures.length}/${run.length} test files passed (${skipped.length} skipped — live runtime + pty journeys; full gate: node scripts/test-all.mjs, runs in CI)`);
