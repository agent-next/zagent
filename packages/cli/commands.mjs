...
export const COMMANDS = [
  ...
  ['permissions [list|revoke <pattern|all>|--reset] [--json]', 'view or revoke remembered always-allow/deny grants', 'Project'],
  ...
  ['goal [list|show|set <text>|pause|resume|clear] [--session id] [--json]', "list sessions, or show/control a session's goal", 'Project'],
  ...
  ['plugins [list] [name] [--json]|install <name> [--json] [--offline]', 'inspect and manage local plugins', 'Extend'],
...
export const COMMAND_DETAILS = {
  ...
  permissions: [
    'Lists the always-allow/deny grants a TUI permission answer remembered,',
    'with the command or input each one covers. Patterns are redacted for',
    'key-shaped secrets only — a credential embedded in a remembered command',
    'stays visible here (grants.json is mode 0600).',
    '  list (default)          every grant as Tool(pattern) — decision',
    '  revoke <pattern|all>    remove grants whose pattern contains the text',
    '                          (or whose tool name matches); "all" clears',
    '  --reset                 same as "revoke all"',
    '  --json                  machine-readable output',
    '',
  ...
  goal: [
    '  list             sessions a goal can be shown for (from the task index; --all includes archived)',
  ...
  plugins: [
    '  plugins [list] [name]      inspect installed/local plugins',
