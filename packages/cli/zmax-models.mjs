...
if (!q) { for (const p of providerList(catalog)) console.log(`${p.id}\t${p.models.length} models\t${p.baseURL ?? ''}`); }
else {
  ...
  if (exact.length) for (const h of exact)
    ...
    let hits = 0;
    for (const p of providerList(catalog)) for (const m of p.models) if (re.test(m)) { console.log(`${p.id}/${m}`); hits++; }
