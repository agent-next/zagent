...
- fix(tui): provision standalone accounts from the cli key when v2/config.json is absent — lifts the kernel's "Model not set" login wall on 3.12+ hosts without a GUI-written config
- chore(release): align package-lock.json with the shipped version
- docs(readme): refresh stale example version string
- fix(release): exempt the sk-dummy123 scenario literal from the escape audit
- feat(release): adjudicated degraded-gateway release mode
